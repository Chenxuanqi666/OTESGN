    ...
    with open(data_path, encoding='utf-8') as infile:
        ...
        for d in data:
            for aspect in d['aspects']:
                ...
                short = list(d['short'])
                # position，计算每个词的相对位置，方面之前的词，位置为 i - aspect['from']，是一个负数
                # 方面词则为0，方面之后的词为正数，
                ...
                # aspect mask [0, 0, 1, 1, 0, 0, 0]，用于记录方面词的位置
                if len(asp) == 0:
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            if opt.parseadj:
                ...
                # 使用 np.pad 将矩阵填充到固定大小 max_length × max_length
...
class Tokenizer4BertGCN:
    def __init__(self, max_seq_len, pretrained_bert_name):
        ...
        # self.tokenizer.max_model_input_sizes = self.max_seq_len
...
class ABSAGCNData(Dataset):
    def __init__(self, fname, tokenizer, opt):
        ...
        polarity_dict = {'Positive': 0, 'Negative': 1, 'Neutral': 2}
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            polarity = polarity_dict[obj['label']]  # 转化为数字索引
            text = obj['text']
            term = obj['aspect']
            ...
            # 方面词在 BERT 输入中出现两次，因此 2 * len(term_tokens)，还有 [CLS] 和两个 [SEP] 标记
            ...
            # 用于区分 BERT 输入中的不同部分（上下文和方面词）[cls] [context] [seq] [asp_word] [SEP] -> [0, 0, 0, 1]
            ...
            # 将词级别的层次度矩阵（obj['short']）扩展到子词级别，处理分词器将一个词拆分为多个子词的情况。
            ...
            # 创建五个 max_length × max_length 的掩码矩阵，并初始化
            mask_0 = [[-99999] * opt.max_length for _ in range(opt.max_length)]
            mask_1 = [[-99999] * opt.max_length for _ in range(opt.max_length)]
            mask_2 = [[-99999] * opt.max_length for _ in range(opt.max_length)]
            mask_3 = [[-99999] * opt.max_length for _ in range(opt.max_length)]
            ...
            # 将所有掩码矩阵的第一行（对应 [CLS] 标记）在上下文范围内（context_len）设为 0
            for i in range(1):
            ...
            # 根据层次距离填充掩码，这些掩码在 GCN 中控制消息传递，仅允许特定层次距离的节点交互
