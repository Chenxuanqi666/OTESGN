...
class GCNAbsaModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 扩展 aspect_mask 的维度，[batch_size, max_seq_len, 100]
...
class GCNBert_with_OT(nn.Module):
    def __init__(self, bert, opt, num_layers):
        ...
        self.attn = MultiHeadAttention(opt.attention_heads, self.attdim, short_dropout=opt.short_drop,
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, h, d_model, short_dropout=0.1, alpha=0.8):
    ...
    def compute_ot_attention(self, sentence_vec, aspect_vec, mask, epsilon=2, max_iter=50, tol=1e-6):
        """
        ...
        K = torch.exp(-cost / epsilon)  # [B, H, L, 1]
    ...
    def attention(self, query, key, short, aspect, bias_m, seq_h, asp_h, mask=None, alpha=0.8):
        ...
        sentence_scores = F.softmax(torch.add(sentence_scores, short), dim=-1)  # [B, h, L, L]
